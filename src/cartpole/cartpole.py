...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    actions = [0, 1]
    values = [np.sum(mdp_data['transition_probs'][state, :, a] * mdp_data['value']) for a in actions]
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for state in range(num_states):
        for action in range(2):
            total_transitions = np.sum(mdp_data['transition_counts'][state, :, action])
            if total_transitions > 0:
                mdp_data['transition_probs'][state, :, action] = mdp_data['transition_counts'][state, :, action] / total_transitions
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    for state in range(num_states):
        actions = [0, 1]
        values = [mdp_data['reward'][state] + gamma * np.sum(mdp_data['transition_probs'][state, :, a] * mdp_data['value']) for a in actions]
        new_value[state] = max(values)

        iteration_diff = max(iteration_diff, abs(new_value[state] - mdp_data['value'][state]))

    mdp_data['value'] = new_value

    return iteration_diff < tolerance
    # *** END CODE HERE ***
