...
def init_centroids(numc, image):
    """
    Initialize a `numc` x image_shape[-1] nparray to RGB
    ...
    """
    H, W, C = image.shape
    nums = np.random.randint(H * W, size=numc)
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for it in range(max_iter):
        ...
        jjj = np.argmin(distance, axis=0).reshape(-1, 1)
        # Do M-step
        new_centroids = np.empty([numc, C])
        for j in range(numc):
            cj = (jjj == j)
            new_centroids[j] = np.sum(cj * image, axis=0) / np.sum(cj)
        # print loss
        if (it + 1) % print_every == 0:
            loss = (image - new_centroids[jjj.squeeze()]) ** 2
            loss = np.sum(loss)
...
def update_image(image, centroids):
    ...
    for j in range(numc):
        distance[j] = np.sum((image - centroids[j]) ** 2, axis=1)
    jjj = np.argmin(distance, axis=0)
    new_image = centroids[jjj].reshape(H, W, C)
    return new_image
...
def main(args):
    ...
    centroids_init = init_centroids(numc, image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numc', type=int, default=16,
                        help='Number of centroids/clusters')
    parser.add_argument('--print_every', type=int, default=10,
                        help='Iteration print frequency')
    args = argparse.Namespace(small_path='./peppers-small.tiff',
